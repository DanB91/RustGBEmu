...
fn readByteFromMemory(memory: &MemoryState, addr: u16) -> u8 {
...
fn writeByteToMemory(memory: &mut MemoryState, byte: u8, addr: u16) {
    let i = addr as usize;
    match addr {
        0xC000...0xDFFF =>
            memory.workingRAM[i - 0xC000] = byte,
            _ => {}
    }
}

fn readWordFromMemory(memory: &MemoryState, addr: u16) -> u16 {
...
fn writeWordToMemory(memory: &mut MemoryState, word: u16, addr: u16 ) {
    debug_assert!(addr + 1 > addr); //check for overflow

    writeByteToMemory(memory, lb(word), addr);
    writeByteToMemory(memory, hb(word), addr+1);
}
fn openROM(fileName: &str) -> Result<Vec<u8>, io::IoError> {

    let mut f = try!(io::File::open(&Path::new(fileName))); 
    Ok(try!(f.read_to_end()))
}
